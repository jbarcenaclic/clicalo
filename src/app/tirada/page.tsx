...
export default function TiradaPage() {
  ...
  const iniciarTirada = async (uid: string) => {
    try {
      ...
      setProgreso((prev) => ({ ...prev, accionesEnCurso: 1 }))
  ...
  const cargarSiguienteAccion = async (tid: string) => {
    try {
      ...
      if (!action) {
        ...
        setTimeout(async () => {
          if (userId && progreso.tiradasCompletadas + 1 < 10) {
            console.log('[tirada] iniciando siguiente tirada')
            await iniciarTirada(userId)
          }
        }, 1000)
